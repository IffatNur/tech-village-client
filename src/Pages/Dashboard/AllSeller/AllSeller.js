...
const AllSeller = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        `https://tech-village-server-iffatnur.vercel.app/allseller`,
        {
          headers: {
  ...
  const handleVerify = (email) => {
    console.log(email);
    fetch(`https://tech-village-server-iffatnur.vercel.app/users/${email}`, {
      method: "PUT",
      headers: {
        authorization: `Bearer ${localStorage.getItem("tech-token")}`,
      },
            ...
            {sellers.map((seller, i) => (
              <tr key={i} className="hover">
                ...
                {seller.isVerified && (
                  <td>
                    <Link>
                      <button className="btn btn-sm border-0 bg-gradient-to-r from-green-600 to-green-900">
                        Verified
                      </button>
                    </Link>
                  </td>
