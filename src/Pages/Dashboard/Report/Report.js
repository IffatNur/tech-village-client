...
const Report = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://tech-village-server-iffatnur.vercel.app/report",
        {
          headers: {
  ...
  const handleRemove = (id, reportId) => {
    ...
    fetch(`https://tech-village-server-iffatnur.vercel.app/product/${id}`, {
      method: "DELETE",
      headers: {
        authorization: `Bearer ${localStorage.getItem("tech-token")}`,
      },
      ...
      .then((data) => {});
    fetch(
      `https://tech-village-server-iffatnur.vercel.app/report/${reportId}`,
      {
        method: "DELETE",
        headers: {
          authorization: `Bearer ${localStorage.getItem("tech-token")}`,
          ...
          <tbody>
            {reports.map((report, i) => (
              <tr key={i} className="hover">
                ...
                <td>
                  <div className="text-sm opacity-50">{report.seller_name}</div>
                </td>
